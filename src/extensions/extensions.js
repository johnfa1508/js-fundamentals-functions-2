...
function estimatePrepTime(ingredientsArray, prepTimePerIngredient) {
  let totalPrepTime = 0
  let timePerIngredient = 0

  prepTimePerIngredient === undefined || null
    ? (timePerIngredient = 2)
    : (timePerIngredient = prepTimePerIngredient)

  for (let i = 0; i < ingredientsArray.length; i++) {
    totalPrepTime += timePerIngredient
  }
...
function calculateQuantities(ingredientsArray, cakeLayers) {
  ...
  if (!ingredientsArray.includes('sugar')) {
    quantity.sugar = 0
  } else if (!ingredientsArray.includes('eggs')) {
    quantity.eggs = 0
  }
...
function improveRecipe(recipeObject, amountOfPortions) {
  const improvedRecipe = {}

  for (const ingredient in recipeObject) {
    improvedRecipe[ingredient] = recipeObject[ingredient] * amountOfPortions
  }
